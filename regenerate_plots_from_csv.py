...
def regenerate_plots(csv_path, schedule=None, output_dir=None):
    ...
    # Generate timestamp suffix from filename or use current time
    timestamp = csv_path.stem.split('_')[-1] if '_' in csv_path.stem else None
    if timestamp is None or len(timestamp) != 15:  # Check if it looks like a timestamp
