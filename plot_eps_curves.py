...
def plot_lhs_rhs_vs_eps(
    ...
    ax.plot(x_plot, y_lhs, marker='o', linestyle='-', label='KL Divergence', 
            linewidth=4, color='darkgrey')
    # ax.plot(x_plot, y_rhs, marker='s', linestyle='-', label='RHS: ε·√S', 
    ax.plot(x_plot, y_rhs, marker='s', linestyle='-', label='Theorem 3.1 Bound', 
    ...
    ax.set_xlabel('ε (RMS flow-matching loss)', fontsize=24)
    ax.set_ylabel('Value', fontsize=24)
    ttl = title or f"KL Error Bound Verification - Schedule {schedule.upper()}"
    ax.set_title(ttl, fontsize=21, fontweight='bold')
